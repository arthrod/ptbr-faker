...
class BrazilianLocationSampler:
    ...
    def format_full_location(self, city: str, state: str, state_abbr: str, include_cep: bool = True, cep_without_dash: bool = False) -> str:
        """Format location with optional CEP"""
        ...
        return f'{base}, {formatted_cep}'

    def get_random_cep(self) -> str:
        """Get a random CEP weighted by city population"""
        state_name, state_abbr, city_name = self.get_state_and_city()
        cep = self._get_random_cep_for_city(city_name)
        return self._format_cep(cep)
    ...
    def get_random_location(
        ...
        """Extended get_random_location with CEP options"""
        if only_cep:
